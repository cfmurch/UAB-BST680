...
#Use theme_set to automatically set the theme for all subsequent plots
theme_set(theme_cfm)
...
cvd_table <- 
  tibble(Variable = colnames(cvd_data))

#Use summarise_all() to get the percentage
prop_missing <- summarise(cvd_data, across(everything(), ~sum(is.na(.) / length(.) * 100)))
...
#Rows and columns of the dataset using the same kable set-up as the first part of question 1
table_curr <-
  tibble(Dim = c("Rows", "Columns"), 
         Type = c("Observations", "Variables"),
         Count = dim(cvd_filter)) |>
...
#Then knit the kable, including a caption  
table_curr_print <-
table_curr |>
  gt() |>
  cols_align("center") |>
...
#Still using the filtered dataset without pre 10-year dropouts, just use mutate followed by if_else commands for the logicals and case_when for the more complex egfr.  End with a mutate_if to cast all character variables as factors
cvd_filter <- cvd_filter |>
  mutate(
    cvd10 = if_else(time_chd_strk<=10 & chd_strk=="Yes", "Yes", "No"),
    albuminuria=if_else(albumin>30, "Yes", "No"),
...
#A little function to help with printing the proportions
percer <- function(xx, yy){
  yy <- round(yy*100)
  out <- str_c(xx, " (", yy, "%)")} 
...
#Once the factor is set, group by gender, get a count within the levels, calculate the frequency, and then use the percer function to get the final string that will be presented in the table
  group_by(sex) |>
  count(med_use, name = "count_curr") |>
  mutate(
    freq_curr = count_curr/sum(count_curr)) |>
  ungroup() |>
  mutate(
    string=percer(count_curr, freq_curr)) |>
  select(sex, "Medication\nProfile"=med_use, string) |>
  spread(sex, string)

#Knit the kable annotating as necessary
