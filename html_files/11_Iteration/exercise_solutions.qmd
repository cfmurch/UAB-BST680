...
dat_curr$pred <- predict(lm(y ~ x, data = dat_curr))

plot_lm <- 
ggplot(dat_curr, aes(x = x, y = y)) +
  geom_segment(aes(x = x, xend = x, y = y, yend = pred), color = "blue", linetype = "dashed") + 
  geom_point(shape = 21, color = "black", fill = "grey", size = 3) + 
  geom_smooth(method = "lm", color="red", linetype = "solid", linewidth = 1.75, se = FALSE) + 
  theme_cfm
plot_lm
...
#saveRDS(dat_curr, "lm_data.rds")
#saveRDS(plot_lm, "solution1.rds")
```
...
est_curr <- c(slope_est, int_est)
```

```{r}
#| include: false

#saveRDS(est_curr, "solution2.rds")
```

Slope estimate from `map()`: `r round(slope_est, 3)`

Intercept estimate from `map()`: `r round(int_est, 3)`
...
#saveRDS(plot_map, "solution3.rds")
...
```{r}
#| include: false
#saveRDS(plot_map2, "solution4.rds")
